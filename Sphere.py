...
class Solve:
    ...
    def sweep(self, n_mu, psi_mu, Phi_0, Phi_1):
        ...
        for iel in range(start, stop, inc):
            ...
            # coefficients
            a00 = 2*np.abs(mu[0])*A_out + alpha[1]*(A[1]-A[0])/(2*w[0])*B_minus(mu_1) + sigt*V
            a01 = alpha[1]*(A[1]-A[0])/(2*w[0])*B_plus(mu_1)
            a10 = (alpha[2]*B_minus(mu_half) - alpha[1]*B_minus(mu_1))*(A[1]-A[0])/(2*w[1])
            a11 = 2*np.abs(mu[1])*A_out + (alpha[2]*B_plus(mu_half) - alpha[1]*B_plus(mu_1)) \
                  *(A[1]-A[0])/(2*w[1]) + sigt*V
                
            # source terms
            b0 = (sigs*Phi_0[iel]+q)/2*V + np.abs(mu[0])*(A[1]+A[0])*psi_x[0] \
                  - (A[1]-A[0])/(2*w[0])*(alpha[1]*B_S(mu_1) - alpha[0])*psi_mu[iel]
...
if error == True:
    ...
    if exact == True:
        if leak == False:
            ...
            sol2 = Solve(R, I_reg, quad_dict2, bc, matprops, do_angular=True)
            sol2.solve()
        ...
        if leak == True:
            ...
            sol3 = Solve(R, I_reg, quad_dict3, bc, matprops)
            leak = sol3.solve()
