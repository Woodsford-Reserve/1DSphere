    ...
    def sweep(self, n_mu, psi_mu, Phi_0, Phi_1):
        ...
        for iel in range(start, stop, inc):
            ...
            if n_mu == 0:
                ...
                b0 += (A[1]-A[0])/(2.*w[0])*alpha[0]*(mu[1]-mu_half[0])/(mu[1]-mu[0])*psi_mu[iel]
                b1 += (A[1]-A[0])/(2.*w[0])*alpha[0]*(mu_half[0]-mu[0])/(mu[1]-mu[0])*psi_mu[iel]
