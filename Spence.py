...
def symmetric1d(N):
    ...
    beta = np.sqrt(0.5*(1. - N/(2.*np.pi) * np.sin(2.*np.pi/N)))
    
    # points (real subinterval)
    phi[0] = -np.arcsin(beta) + np.pi/N
    phi[1] =  np.arcsin(beta) + np.pi/N
    
    # complete quadrature set
    points  = np.zeros((N,2))
    weights = np.pi/N * np.ones((N,2))
    
    # loop over subintervals
    for n in range(N):
        points[n,0] = phi[0] + 2.*np.pi * n/N
        points[n,1] = phi[1] + 2.*np.pi * n/N
...
def integrate1d(integrand, points, weights, N_azimu):
    ...
    integral_true = quad(integrand, 0., 2.*np.pi)[0]
...
def symmetric2D(N_azimu, N_polar):
...
# perform 2D integral
def integrate2d(integrand, prod_quad, N_polar, N_azimu):
    
    # true (global) integral value
    integral_true = dblquad(integrand, 0., 2.*np.pi, -1., 1.)[0]
...
def integrals2d(N, tol=1e-8):
    ...
    for l in range(2*N-1):
        ...
        for m in range(l+1):
            ...
            # generate harmonic
            integrand = lambda xi, phi: lpmv(l,m,xi) * np.cos(m*phi)
            
            # integrate harmonic
            string, error = integrate2d(integrand, prod_quad, N, N)
    ...
    # mu and eta
    mu  = lambda xi, phi: np.sqrt(1. - xi*xi)*np.cos(phi)
    eta = lambda xi, phi: np.sqrt(1. - xi*xi)*np.sin(phi)
    
    # loop over powers of mu
    for i in range(N):
        
        # loop over powers of eta
        for j in range(N - i):
            
            # loop over powers of xi
            for k in range(2*N - i - j):
                
                # generate integrand
                integrand = lambda xi, phi: mu(xi,phi)**i * eta(xi,phi)**j * xi**k
                
                # integrate product of powers
                string, error = integrate2d(integrand, prod_quad, N, N)
                
                # print integral
                if error > tol:
                    print("{:n}"   .format(i).rjust(3) + \
                          "{:n}"   .format(j).rjust(3) + \
                          "{:n} ||".format(k).rjust(6) + string)
    print("\n")
